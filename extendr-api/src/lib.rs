...
unsafe fn make_method_def(cstrings: &mut Vec<std::ffi::CString>, rmethods: &mut Vec<libR_sys::R_CallMethodDef>, func: &metadata::Func) {
...
pub unsafe fn register_call_methods(info: *mut libR_sys::DllInfo, metadata: Metadata) {
    ...
    for func in metadata.functions {
        make_method_def(&mut cstrings, &mut rmethods, &func);
    }
